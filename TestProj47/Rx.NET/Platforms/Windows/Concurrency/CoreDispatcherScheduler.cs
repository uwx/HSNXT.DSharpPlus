...
namespace HSNXT.Reactive.Concurrency
{
    ...
    /// </summary>
    /// <remarks>
    /// This scheduler type is typically used indirectly through the <see cref="HSNXT.Reactive.Linq.DispatcherObservable.ObserveOnDispatcher{TSource}(IObservable{TSource})"/> and <see cref="HSNXT.Reactive.Linq.DispatcherObservable.SubscribeOnDispatcher{TSource}(IObservable{TSource})"/> methods that use the current Dispatcher.
    /// </remarks>
    [CLSCompliant(false)]
            ...
            var res = Dispatcher.RunAsync(Priority, () =>
            {
                if (!d.IsDisposed)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        // in HSNXT.Reactive.PlatformServices\Reactive\Concurrency.
                        //
