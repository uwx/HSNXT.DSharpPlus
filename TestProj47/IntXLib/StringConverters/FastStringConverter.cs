...
namespace HSNXT
{
    ...
    /// </summary>
    internal sealed class FastStringConverter : StringConverterBase
    {
        #region Private fields

        private IStringConverter _classicStringConverter; // classic converter
        ...
        public override unsafe uint[] ToString(uint[] digits, uint length, uint numberBase, ref uint outputLength)
        {
            var outputArray = base.ToString(digits, length, numberBase, ref outputLength);
            ...
            var resultLengthLog2 = Bits.CeilLog2(outputLength);
            var resultLength = 1U << resultLengthLog2;

            // Create and initially fill array for transofmed numbers storing
            var resultArray = ArrayPool<uint>.Instance.GetArray(resultLength);
            Array.Copy(digits, resultArray, length);

            // Create and initially fill array with lengths
            var resultArray2 = ArrayPool<uint>.Instance.GetArray(resultLength);
            resultArray2[0] = length;

            var multiplier = MultiplyManager.GetCurrentMultiplier();
            var divider = DivideManager.GetCurrentDivider();

            // Generate all needed pows of numberBase in stack
            var baseIntStack = new Stack(resultLengthLog2);
            IntX baseInt = default;
            for (var i = 0; i < resultLengthLog2; ++i)
            {
                baseInt = baseInt == default ? numberBase : multiplier.Multiply(baseInt, baseInt);
                baseIntStack.Push(baseInt);
            }

            // Create temporary buffer for second digits when doing div operation
            var tempBuffer = new uint[baseInt.Length];
            ...
            fixed (uint* resultPtr1Const = resultArray, resultPtr2Const = resultArray2, tempBufferPtr = tempBuffer)
            {
                ...
                var resultPtr1 = resultPtr1Const;
                var resultPtr2 = resultPtr2Const;
                    ...
                    innerStep >>= 1, outerStep >>= 1)
                {
                    ...
                    fixed (uint* baseIntPtr = baseInt.Digits)
                    {
