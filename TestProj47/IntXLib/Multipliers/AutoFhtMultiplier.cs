...
namespace HSNXT
{
    ...
    /// </summary>
    internal sealed class AutoFhtMultiplier : MultiplierBase
    {
        #region Private fields

        private IMultiplier _classicMultiplier; // multiplier to use if FHT is unapplicatible
        ...
        public override unsafe uint Multiply(uint* digitsPtr1, uint length1, uint* digitsPtr2, uint length2,
            uint* digitsResPtr)
        {
            ...
            var newLength = length1 + length2;

            // Do FHT for first big integer
            var data1 = FhtHelper.ConvertDigitsToDouble(digitsPtr1, length1, newLength);
            ...
            {
                var lowerDigitCount = Math.Min(length2, Math.Min(length1, Constants.FhtValidityCheckDigitCount));

                // Validate result by multiplying lowerDigitCount digits using classic algorithm and comparing
                var validationResult = new uint[lowerDigitCount * 2];
                fixed (uint* validationResultPtr = validationResult)
                {
                    _classicMultiplier.Multiply(digitsPtr1, lowerDigitCount, digitsPtr2, lowerDigitCount,
                        validationResultPtr);
                    if (DigitOpHelper.Cmp(validationResultPtr, lowerDigitCount, digitsResPtr, lowerDigitCount) != 0)
                    {
                        throw new FhtMultiplicationException(string.Format(Strings.FhtMultiplicationError, length1,
                            length2));
                    }
                }
            }
