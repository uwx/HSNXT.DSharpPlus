...
namespace TestProj47
{
    ...
    /// </summary>
    internal static class NewtonHelper
    {
        /// <summary>
        ...
        public static unsafe uint[] GetIntegerOpposite(
            uint* digitsPtr,
            uint length,
            uint maxLength,
            uint* bufferPtr,
            ...
            out ulong rightShift)
        {
            ...
            var msb = Bits.Msb(digitsPtr[length - 1]);
            rightShift = (ulong) (length - 1) * Constants.DigitBitCount + (ulong) msb + 1U;

            if (msb != 2)
            {
                // Shift to the left (via actually right shift)
                var leftShift = (2 - msb + Constants.DigitBitCount) % Constants.DigitBitCount;
                length =
                    DigitOpHelper.Shr(digitsPtr, length, bufferPtr + 1, Constants.DigitBitCount - leftShift, true) + 1U;
            }
            else
            {
            ...
            var lengthLog2 = Bits.CeilLog2(maxLength);
            var newLengthMax = 1U << (lengthLog2 + 1);
            var lengthLog2Bits = lengthLog2 + Bits.Msb(Constants.DigitBitCount);

            // Create result digits
            var resultDigits = ArrayPool<uint>.Instance.GetArray(newLengthMax); //new uint[newLengthMax];
            uint resultLength;

            // Create temporary digits for squared result (twice more size)
            var resultDigitsSqr = ArrayPool<uint>.Instance.GetArray(newLengthMax); //new uint[newLengthMax];
            uint resultLengthSqr;

            // Create temporary digits for squared result * buffer
            var resultDigitsSqrBuf = new uint[newLengthMax + length];
            ...
            // Fix some digits
            fixed (uint* resultPtrFixed = resultDigits, resultSqrPtrFixed = resultDigitsSqr, resultSqrBufPtr =
                resultDigitsSqrBuf)
            {
                var resultPtr = resultPtrFixed;
                var resultSqrPtr = resultSqrPtrFixed;

                // Cache two first digits
                var bufferDigitN1 = bufferPtr[length - 1];
                var bufferDigitN2 = bufferPtr[length - 2];
                ...
                var nextBufferLength = 1U;
                var nextBufferPtr = &nextBufferTempStorage;
                ...
                for (var k = 0; k < lengthLog2Bits; ++k)
                {
                    // Get result squared
                    ...
                    else
                    {
                        ...
                        nextBufferLength = Math.Min((1U << (k - 4)) + 1U, length);
