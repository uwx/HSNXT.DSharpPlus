namespace HSNXT
{
    ...
    /// </summary>
    internal sealed class AutoNewtonDivider : DividerBase
    {
        #region Private fields

        private IDivider _classicDivider; // divider to use if Newton approach is unapplicatible
        ...
        private bool IsClassicAlgorithmNeeded(uint length1, uint length2)
        {
            return
        ...
        public override unsafe uint DivMod(
            uint[] digits1,
            uint[] digitsBuffer1,
            ref uint length1,
            uint[] digits2,
            ...
            int cmpResult)
        {
            ...
            fixed (uint* digitsPtr1 = digits1, digitsBufferPtr1 = digitsBuffer1, digitsPtr2 =
                digits2, digitsBufferPtr2 = digitsBuffer2 != null ? digitsBuffer2 : digits1, digitsResPtr =
                digitsRes != null ? digitsRes : digits1)
            {
                return DivMod(
                    digitsPtr1,
                    digitsBufferPtr1,
            ...
            int cmpResult)
        {
            ...
            var int2OppositeDigits = NewtonHelper.GetIntegerOpposite(
                digitsPtr2,
                length2,
                length1,
                digitsBufferPtr1,
            ...
            uint quotLength;
            var quotDigits = new uint[length1 + int2OppositeLength];
            ...
            fixed (uint* oppositePtr = int2OppositeDigits, quotPtr = quotDigits)
            {
                ...
                // Calculate shift
                var shiftOffset = (uint) (int2OppositeRightShift / Constants.DigitBitCount);
                var shiftCount = (int) (int2OppositeRightShift % Constants.DigitBitCount);
                ...
                uint quotDivLength;
                var quotDivDigits = new uint[quotLength + length2];
                fixed (uint* quotDivPtr = quotDivDigits)
                {
                    quotDivLength = multiplier.Multiply(quotPtr, quotLength, digitsPtr2, length2, quotDivPtr);

                    var cmpRes = DigitOpHelper.Cmp(quotDivPtr, quotDivLength, digitsPtr1, length1);
                    if (cmpRes > 0)
                    {
                        highestLostBit = 1;
