...
namespace HSNXT
{
    ...
    internal abstract class ParserBase : IParser
    {
        ...
        private IParser _pow2Parser; // parser for pow2 case
        ...
        public virtual IntX Parse(string value, uint numberBase, IDictionary<char, uint> charToDigits, bool checkFormat)
        {
            // Exceptions
            if (value == null)
            {
            ...
            {
                throw new ArgumentException(Strings.ParseBaseInvalid, nameof(numberBase));
            }
            ...
            var negative = false; // number sign
            var stringNotEmpty = false; // true if string is already guaranteed to be non-empty

            // Determine sign and/or base
            var match = ParseRegex.Match(value, startIndex, endIndex - startIndex + 1);
            ...
            var valueLength = endIndex - startIndex + 1;
            var digitsLength = (uint) Math.Ceiling(Math.Log(numberBase) / Constants.DigitBaseLog * valueLength);
            var newInt = new IntX(digitsLength, negative);
        ...
        public virtual uint Parse(string value, int startIndex, int endIndex, uint numberBase,
